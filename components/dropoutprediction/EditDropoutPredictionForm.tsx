...
import { dropoutPredictionSchema } from '@/state/schemas/validationSchemas';
import { z } from 'zod';
...
const EditStudentForm = ({
  ...
  data: {studentId: number} & z.infer<typeof dropoutPredictionSchema>;
}) => {

  const { successResult, errorResult } = useFormSubmitResult();

  const form = useForm<z.infer<typeof dropoutPredictionSchema>>({
    resolver: zodResolver(dropoutPredictionSchema),
    defaultValues: {


      ...dropoutPredictionFormSchema(data)
    },
  });
  ...
  const handleSubmit = (data: z.infer<typeof dropoutPredictionSchema>) => {
    mutate(data);
  };
